...
export function FeaturesSectionDemo({ features }: { features: { title: string, description: string, icon: React.ReactNode }[] }) {
...
const Feature = ({
    ...
    return (
        ...
        >
            {index < 4 && (
                <div className="opacity-0 group-hover/feature:opacity-100 transition duration-200 absolute inset-0 h-full w-full bg-gradient-to-t from-orange-100 dark:from-neutral-800 to-transparent pointer-events-none" />
            )}
            {index >= 4 && (
